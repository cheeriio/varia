...
int main(int argc, char **argv) {
    ...
    if (argc!=9) {
        printf("Wywołanie %s LD_Re, LD_Im, PG_Re, PG_Im, Poziom, Pion, Iteracje dimX(kernel block)\n ",argv[0]);
	    exit(1);
    }
    ...
    printf("Mandelbrot_gpu: dim(%d, 1) ", dimX);
...
int computeMandelbrot(real X0, real Y0, real X1, real Y1, int POZ, int PION, int ITER,int *Mandel ){
    ...
    dim3 threadsPerBlock(dimX,1,1);
    dim3 numBlocks(dimX / 128 + 1, 1, 1);
