...
public class Prospective implements InterfaceProductionStrategy{
    int days;

    public Prospective(int d){days = d;}
    ...
    public void produce(Worker agent, InterfaceMarket market){
        ...
        if(sold[0].size() > days){
            for(int i = 0; i < 4; i++){
                ...
                avg[i] -= money[i].get(money[i].size() - 1 - days) / sold[i].get(sold[i].size() - 1 - days);
            }
        }else if(sold[0].size() > 0){
            ArrayList<Speculator> spec = market.getSpeculators();
            double[] prices = market.getDayZeroPrices();
            for(int i = 0; i < 4; i++){
                avg[i] = money[i].get(money[i].size() - 1) / sold[i].get(sold[i].size() - 1);
                avg[i] -= prices[i] * spec.size();
            }
        }else{
            for(int i = 0; i < 4; i++){
                avg[i] = 0;
