...
public class Greedy implements InterfaceProductionStrategy {
    public void produce(Worker agent, InterfaceMarket market) {
        ...
        avg[0] *= agent.productivity(Typ.CLOTHES, market);
        avg[1] *= agent.productivity(Typ.FOOD, market);
        avg[2] *= agent.productivity(Typ.TOOL, market);
        avg[3] *= agent.productivity(Typ.PROGRAM, market);
        double max = avg[0];
        int index = 0;
        for (int i = 1; i < 4; i++) {
            if (max < avg[i]) {
